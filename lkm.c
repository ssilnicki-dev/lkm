...
module_param(kt_period_jitter_range_param_us, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
MODULE_PARM_DESC(kt_period_jitter_range_us, "Timer jitter range, us");

static int device_read_jitter_range_param_us = 25;
module_param(device_read_jitter_range_param_us, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
...
MODULE_PARM_DESC(use_high_pri_queue, "Use dedicated high priority queue, [1|0]");
...
static ssize_t sysfs_output_current_data(struct kobject*, struct kobj_attribute*, char*);
static struct kobj_attribute sysfs_current_data_attribute = __ATTR(SYSFS_CURRENT_DATA_ENDPOINT, 0444, sysfs_output_current_data, NULL);
...
static ssize_t sysfs_output_device_statistics(struct kobject* kobj, struct kobj_attribute* attr, char* buf)
{
    return sprintf(buf, "device reads: %ld, skips: %ld, data reads: %ld, retries: %ld, errors: %ld\n", atomic_long_read(&device_data_reads), atomic_long_read(&device_read_omissions), atomic_long_read(&data_reads), atomic_long_read(&retries), atomic_long_read(&errors));
}

static inline ssize_t sysfs_output_current_data(struct kobject* kobj, struct kobj_attribute* attr, char* buf)
{
    unsigned sequence;
    long copy_attempts;
    ...
    copy_attempts = 0;
    current_data_buf = kmalloc(DEVICE_DATA_WORDS * sizeof(device_word_t), GFP_KERNEL);
    if (!current_data_buf)
    {
    ...
    do
    {
        ...
        memcpy(current_data_buf, published_device_data, DEVICE_DATA_WORDS * sizeof(device_word_t));
...
static void hardware_device_blocking_read(void)
{
    ...
    memcpy(published_device_data, runtime_data, sizeof(device_word_t) * DEVICE_DATA_WORDS);
...
static enum hrtimer_restart kt_callback(struct hrtimer* timer)
{
    if (likely(kt_period_jitter_range_param_us > 0))
    {
        ...
        kt_period = ktime_set(period_us / USEC_PER_SEC, (period_us % USEC_PER_SEC) * NSEC_PER_USEC);
    }
...
{
    kt_period = ktime_set(kt_period_param_us / USEC_PER_SEC, (kt_period_param_us % USEC_PER_SEC) * NSEC_PER_USEC);
...
static inline int create_sysfs_files(void)
{
    ...
    error = sysfs_create_file(sysfs_kobject, &sysfs_current_data_attribute.attr);
    if (error)
    {
...
static int __init lkm_init(void)
{
    ...
    sysfs_kobject = kobject_create_and_add(SYSFS_DATA_ENDPOINTS_BASE, kernel_kobj);
    if (!sysfs_kobject)
    {
        error = -ENOMEM;
    ...
    {
        high_pri_queue = alloc_workqueue(HIGH_PRIORITY_QUEUE_NAME, WQ_UNBOUND | WQ_HIGHPRI, 0);
        if (!high_pri_queue)
        {
